...
import type { Deployment, ProductId, DeveloperAssignment, ChecklistSection, ChecklistItem, AuditLogEntry, UserRole } from "@/types";
...
/* ─── Users Tab ──────────────────────────────────── */
function UsersTab({ deployments }: { deployments: Deployment[] }) {
  const [userRoles, setUserRoles] = useState<Record<string, UserRole>>({});
  const [saving, setSaving] = useState<string | null>(null);
  const summaries = buildSummaries(deployments);

  useEffect(() => { storage.getUserRoles().then(setUserRoles); }, []);

  const setRole = async (dev: DeveloperSummary, role: UserRole) => {
    setSaving(dev.id);
    await storage.setUserRole(dev.id, role);
    appendAuditLog(null, "user_role_changed", `Changed ${dev.name}'s role to "${role}"`);
    setUserRoles(await storage.getUserRoles());
    setSaving(null);
  };

  const getRole = (id: string): UserRole => userRoles[id] ?? "developer";

  if (summaries.length === 0) return (
    <div className="text-center py-16 text-muted-foreground">
      <Users className="w-10 h-10 mx-auto mb-3 opacity-30" />
      <p className="text-sm">No users yet. Once developers set up their profiles, they appear here.</p>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-3 p-3 rounded-lg border border-border bg-muted/30 text-sm text-muted-foreground">
        ...
        <div>
          <p><strong className="text-foreground">Role-Based Access Control</strong></p>
          <p className="mt-0.5">Assign roles to control what each developer can do. Changes take effect next time they reload their profile.</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-2">
            {(Object.entries(ROLE_UI) as [UserRole, typeof ROLE_UI[UserRole]][]).map(([role, ui]) => (
              <div key={role} className={`flex items-center gap-1.5 text-xs px-2 py-1.5 rounded ${ui.cls}`}>
                {ui.icon}
                <div>
                  <span className="font-semibold">{ui.label}</span>
                  {role === "admin" && <span className="block opacity-70">Full access</span>}
                  {role === "developer" && <span className="block opacity-70">Run deployments</span>}
                  {role === "viewer" && <span className="block opacity-70">Read-only</span>}
                </div>
              </div>
            ))}
          </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">{dev.deployments.length} deployments · last active {dev.lastActivity ? formatDistanceToNow(new Date(dev.lastActivity), { addSuffix: true }) : "never"}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded font-medium ${ui.cls}`}>
                      {ui.icon}{ui.label}
                    </span>
                    <Select value={role} onValueChange={v => setRole(dev, v as UserRole)} disabled={saving === dev.id}>
      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
        ...
        <Tabs defaultValue="overview" className="space-y-5">
          ...
          <TabsContent value="users">
            <UsersTab deployments={deployments} />
          </TabsContent>
